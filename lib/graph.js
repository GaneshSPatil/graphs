...
UndirectedGraph.prototype = {
  ...
  allPaths : function(source, destination, visitedPath, allPaths){
    visitedPath = visitedPath || [];
    allPaths = allPaths || [];
    if(source == destination){
      allPaths.push(visitedPath.concat(source));
      return allPaths;
    };
    for(var i = 0; i < this.graph[source].length; i++){
      if(visitedPath.indexOf(this.graph[source][i]) == -1){
        var paths = this.allPaths(this.graph[source][i], destination, visitedPath.concat(source), allPaths);
        if(paths[paths.length - 1] == destination) allPaths.push(paths);
      };
    }
    return allPaths;   
  }
